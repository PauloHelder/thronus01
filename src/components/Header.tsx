...
import { Search, Bell, Settings, Menu, User, Building, LogOut, ChevronDown, Link2, Activity } from 'lucide-react';
...
const Header: React.FC = () => {
  ...
  const { user, switchChurch, hasRole, hasPermission } = useAuth();
  const [showUserMenu, setShowUserMenu] = useState(false);
  const menuRef = useRef<HTMLDivElement>(null);
  
  const canViewMyChurches = hasRole('admin') || hasRole('superuser') || hasRole('user') || hasPermission('all');
  const canViewSettings = hasRole('admin') || hasRole('superuser') || hasPermission('settings_view') || hasPermission('all');
  const canViewChurchProfile = hasRole('admin') || hasRole('superuser') || hasRole('supervisor') || hasPermission('church_profile_view') || hasPermission('all');
  ...
  const getTitle = () => {
    switch (location.pathname) {
      ...
      case '/my-churches':
      case '/network': return 'Rede de Igrejas';
